...
func TestServiceLoadsFromCache(t *testing.T) {
	tmpDir, err := ioutil.TempDir("", "test-trust-service")
	...
	for role, metaBytes := range alpineMeta {
		assert.NilError(t, ioutil.WriteFile(filepath.Join(imageDir, role.String()+".json"), metaBytes, 0644))
	}

	alpineRef, err := reference.WithName("docker.io/library/alpine")
	assert.NilError(t, err)

	s := NewTrustService(ServiceConfig{
		ContentTrustConfig: &config.ContentTrust{AllowExpiredCachedTrustData: true},
		TrustCachePath:     tmpDir,
		RegistryService: &fakeService{
			err: errors.New("fake registry error"),
		},
	})
	ref, err := s.VerifyImageSigned(context.Background(), alpineRef, nil, nil)
	assert.NilError(t, err)
...
func TestServicePinsOfficialRootKeys(t *testing.T) {
	...
	assertTrustFail := func(s Service) {
		_, err = s.VerifyImageSigned(context.Background(), alpineRef, nil, nil)
		assert.ErrorContains(t, err, "could not validate the path to a trusted root")
	}

	assertTrustSuccess := func(s Service) {
	...
	assertTrustFail(NewTrustService(ServiceConfig{
		ContentTrustConfig: &config.ContentTrust{AllowExpiredCachedTrustData: true},
		TrustCachePath:     tmpDir,
	...
	assertTrustSuccess(NewTrustService(ServiceConfig{
