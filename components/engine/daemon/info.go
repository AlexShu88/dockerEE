...
func (daemon *Daemon) SystemInfo() *types.Info {
	...
	daemon.fillContentTrust(v)
	daemon.fillEnterpriseLicense(v)
...
func (daemon *Daemon) fillContentTrust(v *types.Info) {
	// Only warnings may be added as to not change the API.
	if daemon.configStore.ContentTrust != nil && daemon.configStore.ContentTrust.Mode == config.TrustModePermissive {
