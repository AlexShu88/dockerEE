...
import (
	...
	// crypto/md5 is needed to determine whether the node is FIPS-enabled.
	// Ideally this could be checked in the dockerd CLI command and passed
	// to NewDaemon, but that would require changing the function signature of
	// NewDaemon (and all of its call sites), and we'd like to keep the
	// divergence from CE-engine as minimal as possible.  Hence the import here.
	_ "crypto/md5" // #nosec G501
	"fmt"
	"io/ioutil"
	"net"
	"net/url"
	...
	"github.com/docker/docker/daemon/telemetry"
...
// needed because `config` package is shadowed by config variable in NewDaemon and trying to avoid drift with upstream.
const trustModeDisabled = config.TrustModeDisabled
...
func NewDaemon(ctx context.Context, config *config.Config, pluginStore *plugin.Store) (daemon *Daemon, err error) {
	...
	d := &Daemon{
		...
		fipsEnabled: !crypto.MD5.Available(),
	}
	...
	// build a trust service
	d.trustService = nil
	if config.ContentTrust != nil {
		// if not disabled
		if config.ContentTrust.Mode != "" && config.ContentTrust.Mode != trustModeDisabled {
			d.trustService = trust.NewTrustService(trust.ServiceConfig{
				TrustCachePath:     filepath.Join(config.Root, "trustcache"),
				RegistryService:    registryService,
	...
	if enabled, ok := config.Features["telemetry"]; !ok || (ok && enabled) {
		// telemetry is enabled by default, but can be explicitly enabled/disabled
		// with the "features" configuration option
		logrus.Info("starting telemetry service")
		d.telemetry = telemetry.Start(ctx, d)
	}
...
func (daemon *Daemon) Shutdown() error {
	...
	if daemon.telemetry != nil {
		logrus.Info("stopping telemetry service")
		daemon.telemetry.Stop()
	}
