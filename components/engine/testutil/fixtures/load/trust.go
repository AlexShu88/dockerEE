...
var repositoriesFmt = `{"Repositories":{"busybox":{"busybox:latest":"%s"},"dockertrusttest.docker.io/library/busybox":{"dockertrusttest.docker.io/library/busybox:latest":"%s","dockertrusttest.docker.io/library/busybox@sha256:91ef6c1c52b166be02645b8efee30d1ee65362024f7da41c404681561734c465":"%s"}}}`
...
func FrozenTrustImagesLinux(client client.APIClient, goodTrustMetdadata bool) error {
	...
	for fname, dataBytes := range td {
		if err := ioutil.WriteFile(filepath.Join(info.DockerRootDir, path, fname), dataBytes, 0644); err != nil {
			return err
		}
	}
	...
	err = ioutil.WriteFile(filepath.Join(info.DockerRootDir, "image", info.Driver, "repositories.json"), repositoriesData, 0644)
	return err
}

// WriteConfig writes a custom config file to disk
func WriteConfig(configData string) (string, error) {
	configPath, err := ioutil.TempDir("", "test-daemon-trust")
	if err != nil {
		return "", err
	}
	configFile := filepath.Join(configPath, "config.json")

	err = ioutil.WriteFile(configFile, []byte(configData), 0666)
	if err != nil {
		return "", err
	}
