...
var (
	...
	legacyLicenseNotParseable = []byte(`{"foo":"keyid1`)
	legacyLicenseNotValid     = []byte(`{"key_id":"1", "private_key":"2", "authorization":"3"}`)
...
func TestFillEnterpriseLicense_noLicense(t *testing.T) {
	tmpDir, err := ioutil.TempDir("", "licensing-test")
	assert.NilError(t, err)

	defer func() {
		if err := os.RemoveAll(tmpDir); err != nil {
			t.Fatalf("Unable to remove %s - %v", tmpDir, err)
		}
	}()

	cluster := newTestCluster(true)
	d := &Daemon{root: tmpDir, cluster: cluster}
	v := &types.Info{DockerRootDir: tmpDir}
	v.Swarm.ControlAvailable = false

	assert.Equal(t, "", v.ProductLicense)

	d.fillEnterpriseLicense(v)

	assert.Assert(t, is.Contains(v.ProductLicense, "Unlicensed"))
}

// Ensures that if a license is available, it is properly loaded and reported.
func TestFillEnterpriseLicense_legacyLicenseExpired(t *testing.T) {
	tmpDir, errt := ioutil.TempDir("", "licensing-test")
	assert.NilError(t, errt)
	...
	err := ioutil.WriteFile(licenseFileName, legacyLicenseExpired, 0644)
	assert.NilError(t, err)
...
func TestFillEnterpriseLicense_nonSwarmManager(t *testing.T) {
	tmpDir, errt := ioutil.TempDir("", "licensing-test")
	assert.NilError(t, errt)
...
func TestFillEnterpriseLicenseMirantis_badLicenseParse(t *testing.T) {
	v := &types.Info{}

	err := fillEnterpriseLicenseMirantis(legacyLicenseExpired, v)
	assert.ErrorContains(t, err, "failed to parse")
}

// Ensures that a Mirantis license that is considered invalid fails verification.
func TestFillEnterpriseLicenseMirantis_badLicenseVerify(t *testing.T) {
	v := &types.Info{}

	err := fillEnterpriseLicenseMirantis(mirantisLicenseNotValid, v)
	assert.ErrorContains(t, err, "unable to process")
}

// Ensures that a Mirantis license that is valid (but expired) can be properly
// loaded.
func TestFillEnterpriseLicenseMirantis_validExpired(t *testing.T) {
	v := &types.Info{}
	assert.Assert(t, v.ProductLicense == "")

	err := fillEnterpriseLicenseMirantis(mirantisLicenseValidExpired, v)
	assert.Assert(t, is.Nil(err))
	assert.Assert(t, is.Contains(v.ProductLicense, "Expired"))
}

// Ensures that an unparseable license returns an appropriate error.
func TestFillEnterpriseLicenseLegacy_badLicenseParse(t *testing.T) {
	v := &types.Info{}

	err := fillEnterpriseLicenseLegacy(context.Background(), legacyLicenseNotParseable, v)
	assert.ErrorContains(t, err, "unable to parse")
}

// Ensures that a license that does not verify returns an appropriate error.
func TestFillEnterpriseLicenseLegacy_badLicenseVerify(t *testing.T) {
	v := &types.Info{}

	err := fillEnterpriseLicenseLegacy(context.Background(), legacyLicenseNotValid, v)
	assert.ErrorContains(t, err, "unable to verify")
}

// Ensures that a valid expired license is returned without error, and updates the
// appropriate product license field in info.
func TestFillEnterpriseLicenseLegacy_validExpired(t *testing.T) {
	v := &types.Info{}
...
func TestLoadLicenseCluster_noCluster(t *testing.T) {
	...
	assert.Assert(t, is.Nil(licenseData))
	assert.Assert(t, err != nil)
}
...
func TestLoadLicenseCluster_happyHasLicense(t *testing.T) {
	...
	assert.Assert(t, licenseData != nil)
	assert.Assert(t, is.Nil(err))
}
